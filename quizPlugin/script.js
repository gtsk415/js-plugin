 const quizTemplate = (data = [], dataLength, options) => {
  ...
  const answers = data.answers.map(item => {
    ...
    <label class="quiz-question__label">
      <input type="${item.type}" data-valid="false" class="quiz-question__answer" name="${data.answer_alias}" ${item.type == 'text' ? 'placeholder="Введите ваш вариант"' : '' } value="${item.type !== 'text' ? item.answer__title : ''}">
  ...
  return `
    ...
    <div class="quiz__content">
      ...
      <div class="qiuz-qustion">
        <h3
        class="quiz-question__title">${title}</h3>
 ...
 class Quiz {
  ...
  send() {
    if (this.valid()) {
      ...
      for (let item of this.resulArray ) {
        for( let obj in item) {
